...
from (select *
      from product
               left join product_to_equpment on product.id_product = product_to_equpment.product_id_product) as fir
         left join equpment as sec on equpment_id_equpment = sec.id_equpment
group by equpment_name;
...
select orders_id_orders, id_product
...
select orders_id_orders, sum(materials_price) as price_materials
from (select orders_id_orders, id_product
      from orders_has_product
               left join product p on
          orders_has_product.product_id_product = p.id_product) as a
...
select id_orders, (price_order - price_materials - price_delivery) as profit
...
(select *
 from orders_has_product
          left join product p on p.id_product = orders_has_product.product_id_product);
...
select id_orders, (price_order - price_delivery - equipment_costs - price_materials) as profit
from (select *
      from (select orders_id_orders, sum(materials_price) as price_materials
            from (select orders_id_orders, id_product
                  from orders_has_product
